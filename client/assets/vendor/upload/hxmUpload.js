(function($,undefined){
	$.fn.hxmUpload = function(options,param){
		...
		if (typeof options == 'string') {
			...
			if($.isFunction(fn)){
				...
				throw ("hxmUpload - No such method: " + options);
			}
		}
		...
		return this.each(function(){
			...
			this.funDisposePreviewHtml = function(file, e){
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
				var $uploadList = $("#uploadList_" + file.index);

				if(!para.multiple && $uploadList.length){
					if(file.type.indexOf("image") == 0){
						$uploadList.show().find(".file_name").text(file.name);
						$uploadList.find("#uploadImage_" + file.index).attr("src", e.target.result);
					}
					return html;
				}
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						self.funSetStatusInfo(HXMFILE.funReturnNeedFiles());  // 显示统计信息
						var html = '', i = 0;
						// 组织预览html
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									HXMFILE.funDeleteFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
							...
							if($(".file_edit").length>0){
								...
								$(".file_edit").click(function() {
									// 调用编辑操作
									//HXMFILE.funEditFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
						};
					...
					onSuccess: function(file, response) {
						...
						if(para.finishDel){
							...
							$("#uploadList_" + file.index).fadeOut();
							// 重新设置统计栏信息
							self.funSetStatusInfo(HXMFILE.funReturnNeedFiles());
						}
					},
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(HXMFILE.funReturnNeedFiles().length > 0){
						$("#fileSubmit").click();
					}else{
						alert("请先选中文件再点击上传");
					}
