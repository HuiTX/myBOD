...
(function($, undefined) {
    ...
    Validator.prototype = {
         ...
         */
        isDate: function (str) {
            //匹配: 间隔(- / .), 年4位, 月和日可以是1位. IE8能验证yyyy/MM/dd
            var exp = /^\d{4}(\-|\/|\.)\d{1,2}\1\d{1,2}$/;
            if (exp.test(str) && (!!Date.parse(str) || !!Date.parse(str.replace(/-/g, '/')))) {
                return true; //2月31日会返回true
            } else {
                return false;
            }
        },
        ...
        {
            var exp=/^\d{4}(\-|\/|\.)\d{1,2}\1\d{1,2}\s([01]\d|2[0-3])(:[0-5]\d){1,2}$/;
            return exp.test(str);
        },
    ...
    new Rules({
         ...
         * 验证时间2015-10-12
         */
        myDateTime: function(element, param, field) {
            return this.isDateTime(element.value) || '请输入正确的日期格式';
        },
